...
SHAREGPT_RATES=(4 8 12)
GSM8K_RATES=(5 10 15 20 25 30)
NATURAL_QUESTIONS_RATES=(20)
...
COSPEC_SELECTIVE_VALIDATION_THRESHOLD=(0.1 0.3 0.5 0.7)
...
export BENCHMARK_DURATION=3  # Duration in minutes
...
run_benchmark() {
    ...
    echo "Running benchmark with configuration: $config (Spec Tokens: $spec_tokens, Temperature: $temperature, Request Rate: $request_rate, Dataset: $dataset, Selective Validation Threshold: $COSPEC_SELECTIVE_VALIDATION_THRESHOLD)"
    ...
    echo "$config,$spec_tokens,$temperature,$request_rate,$dataset,$TENSOR_PARALLEL_SIZE,$DRAFT_TENSOR_PARALLEL_SIZE,$COSPEC_SELECTIVE_VALIDATION_THRESHOLD,${results[*]}" | tr ' ' ',' >> "$RESULTS_DIR/benchmark_results.csv"
}
...
for config in "${CONFIG_ORDER[@]}"; do
    ...
    # Loop through selective validation thresholds
    for threshold in "${COSPEC_SELECTIVE_VALIDATION_THRESHOLD[@]}"; do
        echo "Running with selective validation threshold: $threshold"
        # Set the threshold in environment variable
        export COSPEC_SELECTIVE_VALIDATION_THRESHOLD=$threshold
