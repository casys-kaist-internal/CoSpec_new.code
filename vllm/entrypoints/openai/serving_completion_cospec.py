...
class OpenAIServingCompletionCoSpec(OpenAIServing):
    ...
    async def profile_tiling(self):
        ...
        # set max_num_seqs to max_num_batched_tokens just for profiling. 
        await self.engine_client.set_num_speculative_tokens(0)
        await self.engine_client.set_max_num_seqs(max_num_batched_tokens)

        total_iterations = max_num_batched_tokens // 8

        # Warmup
        await self._profile_tiling(512)

        await self.engine_client.start_cospec_profile(mode="tiling")

        with tqdm(total=total_iterations, desc="Profiling...") as pbar:
            for batch_size in range(8, max_num_batched_tokens + 1, 8):
                await self._profile_tiling(batch_size)
                pbar.update(1)
    ...
    async def _profile_tiling(self, batch_size: int):
        generators: list[AsyncGenerator[RequestOutput, None]] = []

        for i in range(batch_size):
            request_id = f"profile_{i}"
            dummy_prompt = TokensPrompt(prompt_token_ids=[1])
            sampling_params = SamplingParams(temperature=0, ignore_eos=True, max_tokens=16)
            generator = self.engine_client.generate(prompt=dummy_prompt, sampling_params=sampling_params, request_id=request_id)
            generators.append(generator)
