...
class SelectiveValidator:
    ...
    def selective_validation(self, proposals: SpeculativeProposals) -> SpeculativeProposals:
        ...
        """
        if proposals.no_proposals or proposals.pre_temperature_probs is None or not self.is_model_trained:
            return proposals
        ...
        proposals.no_proposals = not valid_mask.any()
    ...
    def update_proposal_history(self, proposals: SpeculativeProposals, proposal_scores: SpeculativeScores):
        ...
        """
        if proposals.no_proposals or proposals.pre_temperature_probs is None:
            return
        ...
        pre_temperature_probs = proposals.pre_temperature_probs

        # Update history and train model if needed
        self._update_history(pre_temperature_probs, acceptance_probs)
    ...
    def predict_acceptance_probability(self, proposals: SpeculativeProposals) -> torch.Tensor:
        ...
        pre_temperature_probs_np = pre_temperature_probs.cpu().numpy()
        original_shape = pre_temperature_probs_np.shape
        
        # Reshape for prediction
        pre_temperature_probs_np = pre_temperature_probs_np.reshape(-1, 1)
        
        # Get predictions and clip to valid range
        predictions = self.regression_model.predict(pre_temperature_probs_np)
        ...
        # Convert to tensor
        predictions = torch.from_numpy(predictions).to(pre_temperature_probs.device)
    ...
    def _update_history(self, pre_temperature_probs, acceptance_probs):
        """Update the history of proposal acceptance data and train model if enough data is available.
        
        Args:
            pre_temperature_probs: Tensor of pre-temperature probabilities
        ...
        pre_temperature_probs_np = pre_temperature_probs.cpu().numpy().flatten()
        acceptance_probs_np = acceptance_probs.cpu().numpy().flatten()

        # Add new data to history
        self.history_X.extend(pre_temperature_probs_np)
