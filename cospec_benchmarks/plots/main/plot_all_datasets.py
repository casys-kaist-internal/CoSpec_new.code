...
# Model pairs for subplot labels
model_pairs = [
    '(a) OPT-6.7B/OPT-125M (A6000)',
    '(b) Llama-13B/Vicuna-68M (A6000x2)',
    '(c) OPT-13B/OPT-125M (A100)',
    '(d) OPT-66B/OPT-1.3B (A100x4)',
    '(e) OPT-30B/OPT-350M (H200)'
]

# Reference values for each dataset (latency, throughput)
reference_values = {
    'A.csv': (500, 2),  # (latency_ms, throughput_req_s)
    'B.csv': (1000, 4),
    'C.csv': (1000, 2),
    'D.csv': (500, 1),
    'E.csv': (500, 2)
...
# Y-axis limits for each dataset
y_limits = {
    'A.csv': (5, 600),
    'B.csv': (5, 2300),
    'C.csv': (5, 1500),
...
# X-axis limits for each dataset
x_limits = {
    'A.csv': (0, 12),
    'B.csv': (0, 17.5),
    'C.csv': (0, 10),
...
def create_plot(csv_file, model_label, reference_latency, reference_throughput, x_lim, y_lim):
    """Create a single plot for the given dataset"""
    # Read the CSV file
    ...
    ax.text(0.5, -0.25, model_label, transform=ax.transAxes, 
    ...
    output_filename = f'{base_name}_only.pdf'
...
for i, csv_file in enumerate(CSV_FILES):
    ...
    reference_latency, reference_throughput = reference_values[csv_file]
    y_lim = y_limits[csv_file]
    x_lim = x_limits[csv_file]
    model_label = model_pairs[i]
    
    create_plot(csv_file, model_label, reference_latency, reference_throughput, x_lim, y_lim)
