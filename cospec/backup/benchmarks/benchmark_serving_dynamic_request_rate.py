...
async def get_request(
    ...
    dynamic_request_rates: Optional[list[tuple[float, float]]] = None,
    ...
    Args:
        ...
        dynamic_request_rates:
            List of (duration, rate) tuples for dynamic rate changes
        benchmark_start_time:
            Start time of the benchmark for rate change calculations
    """
    input_requests: Iterable[SampleRequest] = iter(input_requests)
    current_rate = request_rate
    current_rate_idx = 0
    current_rate_start_time = benchmark_start_time if benchmark_start_time else time.perf_counter()
    ...
    for request in input_requests:
        # Check if we need to change the request rate
        if dynamic_request_rates and benchmark_start_time:
            current_time = time.perf_counter()
            if current_rate_idx < len(dynamic_request_rates) - 1:
                next_rate_start = current_rate_start_time + dynamic_request_rates[current_rate_idx][0]
                if current_time >= next_rate_start:
                    current_rate_idx += 1
                    current_rate_start_time = current_time
                    current_rate = dynamic_request_rates[current_rate_idx][1]
                    theta = 1.0 / (current_rate * burstiness)
                    print(f"Time {current_time - benchmark_start_time:.1f}s: Changing request rate to: {current_rate} req/s")
...
def save_token_latencies_to_csv(token_latencies: list[tuple[float, float]], filename: str):
    """Save token latencies and request arrival times to a CSV file.
    
    Args:
        token_latencies: List of tuples containing (arrival_time, token_latency)
        filename: Output CSV filename
    """
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['request_index', 'arrival_time_s', 'token_latency_ms'])
        for i, (arrival_time, latency) in enumerate(token_latencies):
            writer.writerow([i, arrival_time, latency])
...
async def benchmark(
    ...
    dynamic_request_rates: Optional[list[tuple[float, float]]] = None,  # List of (duration, rate) tuples
):
    ...
    tasks: list[tuple[asyncio.Task, float]] = []  # Store (task, arrival_time) tuples
    ordered_token_latencies: list[tuple[float, float]] = []  # List of (arrival_time, latency) tuples
    
    # Create an infinite iterator of requests
    request_iterator = itertools.cycle(input_requests)
    
    if dynamic_request_rates:
        total_duration = sum(duration for duration, _ in dynamic_request_rates)
        end_time = benchmark_start_time + total_duration
        
        print(f"Starting benchmark with dynamic request rates:")
        for i, (duration, rate) in enumerate(dynamic_request_rates):
            print(f"Phase {i+1}: {rate} req/s for {duration} seconds")
        print(f"Total duration: {total_duration} seconds")
        
        request_rate = dynamic_request_rates[0][1]
        
        async for request in get_request(request_iterator, request_rate, burstiness, 
                                      dynamic_request_rates, benchmark_start_time):
            ...
            # Record arrival time when creating the task
            arrival_time = time.perf_counter() - benchmark_start_time
            task = asyncio.create_task(
                limited_request_func(request_func_input=request_func_input,
                                     pbar=pbar))
            tasks.append((task, arrival_time))
    else:
        # Original behavior for single request rate
        if duration_minutes:
        ...
        async for request in get_request(request_iterator, request_rate, burstiness):
            ...
            task = asyncio.create_task(
                limited_request_func(request_func_input=request_func_input,
                                     pbar=pbar))
            tasks.append((task, time.perf_counter() - benchmark_start_time))
    ...
    # Gather completed tasks and collect token latencies in order
    outputs = []
    for task, arrival_time in tasks:
        try:
            output = await task
            outputs.append(output)
            if output.success:
                token_latency = output.latency / output.output_tokens if output.output_tokens > 0 else 0
                ordered_token_latencies.append((arrival_time, token_latency * 1000))  # Convert to ms
    ...
    # Save token latencies to CSV
    if ordered_token_latencies:
        current_dt = datetime.now().strftime("%Y%m%d-%H%M%S")
        csv_filename = f"token_latencies_{current_dt}.csv"
        save_token_latencies_to_csv(ordered_token_latencies, csv_filename)
        print(f"Saved token latencies to {csv_filename}")
...
def main(args: argparse.Namespace):
    ...
    benchmark_result = asyncio.run(
        benchmark(
            ...
            dynamic_request_rates=[(60, 4), (60, 10), (60, 4)] if args.dynamic_rates else None,
        ))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dynamic-rates",
        action="store_true",
        help="Run benchmark with dynamic request rates: 4 req/s for 60s, "
        "8 req/s for 60s, then 4 req/s for 60s",
    )
