...
export DISABLE_BY_BATCH_SIZE=4
...
# Speculative config
# if [ "$AR" -eq 0 ]; then
#     CMD+=" --speculative_config '{\"model\": \"$DRAFT_MODEL\", \"num_speculative_tokens\": $NUM_SPEC_TOKENS, \"draft_tensor_parallel_size\": $DRAFT_TENSOR_PARALLEL_SIZE, \"disable_by_batch_size\": $DISABLE_BY_BATCH_SIZE}'"
# else
if [ "$AR" -eq 0 ]; then
